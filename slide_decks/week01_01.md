...
## (some of) What this course is, as one example:

- You need a 1-mile buffer around every state park in Ohio. That's... sixty-seven parks.

- **Making one ene map is fine:** open the tool, fill in the dialog, click run

- **Making five hundred maps?** one per county, one per year, one per client — clicking through that same dialog five hundred times is less fine. Slow, error-prone, and soul-crushing

- Programming is how we shift from "I did this once" to "I can do this reliably, on any input, as many times as I need to"
...
*This course is about building reproducible, auditable workflows. It's not just about programming for its own sake*
...
If you want to work outside the lab, be aware that ArcGIS Pro only runs on Windows. But the open-source portion of the course will run entirely in the browser via Colab, so it works on any machine with an internet connection
...
# Weekly(ish) assigments: Labs and sketches
...
After the submission deadline, you'll be matched with a classmate's submission. You review their against three questions: what does it do well, where would it break or mislead, and what's one concrete improvement.
...
- We are going to use them **critically** — eventually as a source of a first draft to verify, test, and understand
...
1. **Type your code.** Do NOT copy-paste from the notebook. 
